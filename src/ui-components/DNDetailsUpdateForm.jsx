...
import { getDNDetails } from "../graphql/queries";
...
export default function DNDetailsUpdateForm(props) {
  const {
    id: idProp,
    dNDetails: dNDetailsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = dNDetailsRecord
      ? { ...initialValues, ...dNDetailsRecord }
      : initialValues;
    setEmpID(cleanValues.empID);
    setDoeEmpSubmit(cleanValues.doeEmpSubmit ?? []);
    setCurrentDoeEmpSubmitValue("");
    setDoeEmpApproval(cleanValues.doeEmpApproval ?? []);
    setCurrentDoeEmpApprovalValue("");
    setDoeEmpValid(cleanValues.doeEmpValid ?? []);
    setCurrentDoeEmpValidValue("");
    setDoeEmpRefNo(cleanValues.doeEmpRefNo ?? []);
    setCurrentDoeEmpRefNoValue("");
    setDoeEmpUpload(cleanValues.doeEmpUpload ?? []);
    setCurrentDoeEmpUploadValue("");
    setPermitType(cleanValues.permitType ?? []);
    setCurrentPermitTypeValue("");
    setNlmsEmpSubmit(cleanValues.nlmsEmpSubmit ?? []);
    setCurrentNlmsEmpSubmitValue("");
    setNlmsEmpSubmitRefNo(cleanValues.nlmsEmpSubmitRefNo ?? []);
    setCurrentNlmsEmpSubmitRefNoValue("");
    setNlmsEmpApproval(cleanValues.nlmsEmpApproval ?? []);
    setCurrentNlmsEmpApprovalValue("");
    setNlmsRefNo(cleanValues.nlmsRefNo ?? []);
    setCurrentNlmsRefNoValue("");
    setNlmsEmpValid(cleanValues.nlmsEmpValid ?? []);
  ...
  const [dNDetailsRecord, setDNDetailsRecord] =
    React.useState(dNDetailsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getDNDetails.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getDNDetails
        : dNDetailsModelProp;
      setDNDetailsRecord(record);
    };
    queryData();
  }, [idProp, dNDetailsModelProp]);
  React.useEffect(resetStateValues, [dNDetailsRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          empID,
          doeEmpSubmit: doeEmpSubmit ?? null,
          doeEmpApproval: doeEmpApproval ?? null,
          doeEmpValid: doeEmpValid ?? null,
          doeEmpRefNo: doeEmpRefNo ?? null,
          doeEmpUpload: doeEmpUpload ?? null,
          permitType: permitType ?? null,
          nlmsEmpSubmit: nlmsEmpSubmit ?? null,
          nlmsEmpSubmitRefNo: nlmsEmpSubmitRefNo ?? null,
          nlmsEmpApproval: nlmsEmpApproval ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateDNDetails.replaceAll("__typename", ""),
            variables: {
              input: {
                id: dNDetailsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "DNDetailsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Emp id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || dNDetailsModelProp)}
