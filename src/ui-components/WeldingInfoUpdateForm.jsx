...
import { getWeldingInfo } from "../graphql/queries";
...
export default function WeldingInfoUpdateForm(props) {
  const {
    id: idProp,
    weldingInfo: weldingInfoModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = weldingInfoRecord
      ? { ...initialValues, ...weldingInfoRecord }
      : initialValues;
    setEmpID(cleanValues.empID);
    setDiameterRange(cleanValues.diameterRange);
    setFillerMetal(cleanValues.fillerMetal);
    setThicknessRange(cleanValues.thicknessRange);
    setWeldingStampNor(cleanValues.weldingStampNor);
    setWpsNumber(cleanValues.wpsNumber);
    setWeldingProcess(cleanValues.weldingProcess);
    setWeldingPosition(cleanValues.weldingPosition);
    setWQExpiry(cleanValues.WQExpiry);
    setWQRemarks(cleanValues.WQRemarks);
    setWeldingUpload(cleanValues.weldingUpload ?? []);
    setCurrentWeldingUploadValue("");
    setWQRNo(cleanValues.WQRNo);
    setWeldingCode(cleanValues.weldingCode);
    setWeldingMaterial(cleanValues.weldingMaterial);
    setErrors({});
  };
  const [weldingInfoRecord, setWeldingInfoRecord] =
    React.useState(weldingInfoModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getWeldingInfo.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getWeldingInfo
        : weldingInfoModelProp;
      setWeldingInfoRecord(record);
    };
    queryData();
  }, [idProp, weldingInfoModelProp]);
  React.useEffect(resetStateValues, [weldingInfoRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          empID,
          diameterRange: diameterRange ?? null,
          fillerMetal: fillerMetal ?? null,
          thicknessRange: thicknessRange ?? null,
          weldingStampNor: weldingStampNor ?? null,
          wpsNumber: wpsNumber ?? null,
          weldingProcess: weldingProcess ?? null,
          weldingPosition: weldingPosition ?? null,
          WQExpiry: WQExpiry ?? null,
          WQRemarks: WQRemarks ?? null,
          weldingUpload: weldingUpload ?? null,
          WQRNo: WQRNo ?? null,
          weldingCode: weldingCode ?? null,
          weldingMaterial: weldingMaterial ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateWeldingInfo.replaceAll("__typename", ""),
            variables: {
              input: {
                id: weldingInfoRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "WeldingInfoUpdateForm")}
      {...rest}
    >
      <TextField
        label="Emp id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || weldingInfoModelProp)}
