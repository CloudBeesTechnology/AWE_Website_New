...
import { getLeaveStatus } from "../graphql/queries";
...
export default function LeaveStatusUpdateForm(props) {
  const {
    id: idProp,
    leaveStatus: leaveStatusModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = leaveStatusRecord
      ? { ...initialValues, ...leaveStatusRecord }
      : initialValues;
    ...
    setApplyTo(cleanValues.applyTo ?? []);
  ...
  const [leaveStatusRecord, setLeaveStatusRecord] =
    React.useState(leaveStatusModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getLeaveStatus.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getLeaveStatus
        : leaveStatusModelProp;
      setLeaveStatusRecord(record);
    };
    queryData();
  }, [idProp, leaveStatusModelProp]);
  React.useEffect(resetStateValues, [leaveStatusRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateLeaveStatus.replaceAll("__typename", ""),
            variables: {
              input: {
                id: leaveStatusRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "LeaveStatusUpdateForm")}
      {...rest}
    >
      <TextField
        label="Emp id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || leaveStatusModelProp)}
