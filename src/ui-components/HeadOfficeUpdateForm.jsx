...
import { getHeadOffice } from "../graphql/queries";
import { updateHeadOffice } from "../graphql/mutations";
...
export default function HeadOfficeUpdateForm(props) {
  const {
    id: idProp,
    headOffice: headOfficeModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = headOfficeRecord
      ? { ...initialValues, ...headOfficeRecord }
      : initialValues;
  ...
  const [headOfficeRecord, setHeadOfficeRecord] =
    React.useState(headOfficeModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getHeadOffice.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getHeadOffice
        : headOfficeModelProp;
    ...
    queryData();
  }, [idProp, headOfficeModelProp]);
  React.useEffect(resetStateValues, [headOfficeRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateHeadOffice.replaceAll("__typename", ""),
            variables: {
              input: {
                id: headOfficeRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "HeadOfficeUpdateForm")}
      {...rest}
    >
      <ArrayField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || headOfficeModelProp)}
