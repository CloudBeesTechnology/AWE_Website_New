...
import { getORMCSheet } from "../graphql/queries";
import { updateORMCSheet } from "../graphql/mutations";
...
export default function ORMCSheetUpdateForm(props) {
  const {
    id: idProp,
    oRMCSheet: oRMCSheetModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = oRMCSheetRecord
      ? { ...initialValues, ...oRMCSheetRecord }
      : initialValues;
  ...
  const [oRMCSheetRecord, setORMCSheetRecord] =
    React.useState(oRMCSheetModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getORMCSheet.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getORMCSheet
        : oRMCSheetModelProp;
      setORMCSheetRecord(record);
    };
    queryData();
  }, [idProp, oRMCSheetModelProp]);
  React.useEffect(resetStateValues, [oRMCSheetRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateORMCSheet.replaceAll("__typename", ""),
            variables: {
              input: {
                id: oRMCSheetRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ORMCSheetUpdateForm")}
      {...rest}
    >
      <TextField
        label="Date"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || oRMCSheetModelProp)}
