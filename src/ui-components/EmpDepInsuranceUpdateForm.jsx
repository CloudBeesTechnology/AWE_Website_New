...
import { getEmpDepInsurance } from "../graphql/queries";
...
export default function EmpDepInsuranceUpdateForm(props) {
  const {
    id: idProp,
    empDepInsurance: empDepInsuranceModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = empDepInsuranceRecord
      ? { ...initialValues, ...empDepInsuranceRecord }
      : initialValues;
    setEmpID(cleanValues.empID);
    setDepInsurance(cleanValues.depInsurance ?? []);
    setCurrentDepInsuranceValue("");
    setErrors({});
  };
  const [empDepInsuranceRecord, setEmpDepInsuranceRecord] = React.useState(
    empDepInsuranceModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getEmpDepInsurance.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getEmpDepInsurance
        : empDepInsuranceModelProp;
      setEmpDepInsuranceRecord(record);
    };
    queryData();
  }, [idProp, empDepInsuranceModelProp]);
  React.useEffect(resetStateValues, [empDepInsuranceRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          empID: empID ?? null,
          depInsurance: depInsurance ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateEmpDepInsurance.replaceAll("__typename", ""),
            variables: {
              input: {
                id: empDepInsuranceRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "EmpDepInsuranceUpdateForm")}
      {...rest}
    >
      <TextField
        label="Emp id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || empDepInsuranceModelProp)}
