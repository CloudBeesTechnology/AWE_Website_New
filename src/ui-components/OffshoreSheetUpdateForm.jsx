...
import { getOffshoreSheet } from "../graphql/queries";
import { updateOffshoreSheet } from "../graphql/mutations";
...
export default function OffshoreSheetUpdateForm(props) {
  const {
    id: idProp,
    offshoreSheet: offshoreSheetModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = offshoreSheetRecord
      ? { ...initialValues, ...offshoreSheetRecord }
      : initialValues;
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getOffshoreSheet.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getOffshoreSheet
        : offshoreSheetModelProp;
      setOffshoreSheetRecord(record);
    };
    queryData();
  }, [idProp, offshoreSheetModelProp]);
  React.useEffect(resetStateValues, [offshoreSheetRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateOffshoreSheet.replaceAll("__typename", ""),
            variables: {
              input: {
                id: offshoreSheetRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "OffshoreSheetUpdateForm")}
      {...rest}
    >
      <TextField
        label="Date"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || offshoreSheetModelProp)}
